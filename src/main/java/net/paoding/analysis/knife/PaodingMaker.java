...
public class PaodingMaker {
	...
	private static Log log = LogFactory.getLog(net.paoding.analysis.knife.PaodingMaker.class);
...
//	private static Dictionaries outDictionaries = null;
	...
	public static void setAnalyzerListener(PaodingAnalyzerListener listener) {
		net.paoding.analysis.knife.PaodingMaker.listener = listener;
	}
	...
	private static void setDicHomeProperties(Properties p) {
		...
		if (dicHome == null) {
			...
			if (f.exists()) {
				...
				URL url = net.paoding.analysis.knife.PaodingMaker.class.getClassLoader()
						.getResource("dic");
				if (url != null) {
	...
	private static Paoding createPaodingWithKnives(Properties p)
		...
		// 寻找传说中的Knife。。。。
		final Map<String, net.paoding.analysis.knife.Knife> knifeMap = new HashMap<String, net.paoding.analysis.knife.Knife>();
		final List<net.paoding.analysis.knife.Knife> knifeList = new LinkedList<net.paoding.analysis.knife.Knife>();
		...
		while (iter.hasNext()) {
			...
			if (index == 0 && key.length() > Constants.KNIFE_CLASS.length()) {
				...
				if (end == -1) {
					...
					net.paoding.analysis.knife.Knife knife = (net.paoding.analysis.knife.Knife) clazz.newInstance();
					knifeList.add(knife);
						...
						public void run() throws Exception {
							...
							if (!obj
								...
								setWrappedInstance.invoke(beanWrapperImpl,
										new Object[] { obj });
								knifeMap.put(knifeName, (net.paoding.analysis.knife.Knife) beanWrapperImpl);
								obj = beanWrapperImpl;
							}
	...
	private static ClassLoader getClassLoader() {
		...
		if (loader == null) {
			loader = net.paoding.analysis.knife.PaodingMaker.class.getClassLoader();
		}
		return loader;
	}
